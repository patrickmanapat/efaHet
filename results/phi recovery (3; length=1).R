...
for (i in 1:nrow(df1)) {
  ...
  if(df1$nEta[i] > 1) {
    ...
    matPhi <- read.table(paste0('./path 1/',K,'/factor correlations/','nEta',nEta,'pEta',pEta,'rEta',rEta,'l',l,'n',n,'d',d,'.dat'),
                         header = FALSE,
                         sep = '\t')
...
for (i in 1:nrow(df2)) {
  ...
  if(df2$nEta[i] > 1) {
    ...
    #read in estimated factor correlations
    matPhi <- read.table(paste0('./path 2/',K,'/factor correlations/','nEta',nEta,'pEta',pEta,'rEta',rEta,'l',l,'n',n,'d',d,'.dat'),
                         header = FALSE,
                         sep = '\t')
...
df4 <- filter(df, efa == 2 & rot == 1 & length == 1) %>%
  mutate(biasMean = NA,
         RMSE = NA)
...
for (i in 1:nrow(df4)) {
  ...
  if(df4$nEta[i] > 1) {
    ...
    #read in estimated factor correlations
    matPhi <- read.table(paste0('./path 4/',K,'/factor correlations/','nEta',nEta,'pEta',pEta,'rEta',rEta,'l',l,'n',n,'d',d,'.dat'),
                         header = FALSE,
                         sep = '\t')
...
df5 <- filter(df, efa == 2 & rot == 2 & length == 1) %>%
  mutate(biasMean = NA,
         RMSE = NA)
...
for (i in 1:nrow(df5)) {
  ...
  if(df5$nEta[i] > 1) {
    ...
    #read in estimated factor correlations
    matPhi <- read.table(paste0('./path 5/',K,'/factor correlations/','nEta',nEta,'pEta',pEta,'rEta',rEta,'l',l,'n',n,'d',d,'.dat'),
                         header = FALSE,
                         sep = '\t')
...
df6 <- filter(df, efa == 2 & rot == 3 & length == 1) %>%
  mutate(biasMean = NA,
         RMSE = NA)
